...
async def main():
    ...
    if sys.platform == "emscripten":
        ...
        track = platform.window.MM.prepare("live/PenguinsCantFly.apk", json.dumps(cfg))
    ...
    try:
        if sys.platform == "emscripten":           
            first_high = int( json.loads( window.localStorage.getItem("mygame") or '0') )
        else:
    ...
    while run:
        ...
        for i in range(len(enemy_boxes)):
            if player.colliderect(enemy_boxes[i]) and not game_over:
                ...
                if score > first_high:
                    if sys.platform == "emscripten":
                        window.localStorage.setItem("mygame",  json.dumps(score) )
                    else:
